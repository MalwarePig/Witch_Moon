...
public class LibroRecetas : MonoBehaviour
{
    ...
    string[] Ingredientes = { "Vacio", "Potion_Purple", "Potion_Blue", "Potion_Red", "PolvoAzul", "PolvoRed" };
    ...
    public void AgregarEspecie()
    {
       ...
       //Debug.Log("El Nombre que llega al libro: " + IngredienteActual);
    }
    ...
    public void CrearRecetaRandom()
    {
        ...
        Receta_Uno = new Dictionary<string, bool>(){
           { ingrActual[0], false },
           { ingrActual[1], false },
           { ingrActual[2], false },
           { ingrActual[3], false },
    ...
    public void MostrarReceta()//Muestra los iconos segun la receta
    {
        ...
        foreach (var entry in Receta_Uno)
        {
             //Debug.Log(entry.Key + ":" + entry.Value);
             switch(entry.Key){
                case "PolvoAzul":  MarcoIcono[index].GetComponent<Image>().sprite = SpritePotion[0];
                break;
                case "PolvoRed":  MarcoIcono[index].GetComponent<Image>().sprite = SpritePotion[1];
                break;
                case "Potion_Blue":  MarcoIcono[index].GetComponent<Image>().sprite = SpritePotion[2];
                break;
                case "Potion_Purple":  MarcoIcono[index].GetComponent<Image>().sprite = SpritePotion[3];
                break;
                case "Potion_Red":  MarcoIcono[index].GetComponent<Image>().sprite = SpritePotion[4];
                break; 
